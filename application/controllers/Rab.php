...
class Rab extends CI_Controller{
    ...
    function history(){      
        ...
        foreach ($history as $key => $dt) {
            if ($dt->status == '0' || $dt->status == '1' || $dt->status == '4' || $dt->status == '6' || $dt->status == '7') {
                $proses += 1;
            } 
            if ($dt->status == '1') {
                $vp += 1;
        ...
        $this->load->view('page/rab/rab_history',$data);
    ...
    function create(){
        ...
        $this->load->view('frame/a_header');
        $this->load->view('frame/b_nav',$data);
        $this->load->view('page/rab/rab_create',$data);
        $this->load->view('frame/d_footer');   
    }

    function getCoasAPI(){
        $coas = [];
        $ipr = $this->input->post('id_project');
        $year = date('Y');
        $api = json_decode($this->APICoreRKA->Apiget($this->url."/coa/project/".$ipr."/".$year));
        if ($api != null && $api->code == "1") {
            $coas = $api->data;
    ...
    function getCoas(){
        $coas = [];
        $coas_dmy = $this->session->userdata('coas');
        $nm_coa = "";
        foreach ($coas_dmy as $coa) {
           if ($coa->no_coa != $nm_coa) {
               $coas [] = $coa;
               $nm_coa = $coa->no_coa;
           }
        }
    ...
    function getProjectByID(){
        $projects = [];
        $ipr = $this->input->post('ipr');
        $projects_dmy = $this->session->userdata('projects');
        foreach ($projects_dmy as $pro) {
            if ($pro->id_project == $ipr) {
                $projects [] = $pro;
                break;   
            }
        }
    ...
    function getPegawaiByID(){
        $pegawais = [];
        $ipg = $this->input->post('ipg');
        $pegawais_dmy = $this->session->userdata('pegawais');
        foreach ($pegawais_dmy as $peg) {
            if ($peg->iduser == $ipg) {
                $pegawais [] = $peg;
                break;   
            }
    ...
    function save(){
        ...
        //SIMPEN DULU FILE NYA TERUS AMBIL PATH NYA (BELUM)
        // $tmpFile = $_FILES['add_file']['tmp_name'];
        // $typeFile = $_FILES['add_file']['type'];
        // $nameFile = $_FILES['add_file']['name'];
        // $path_upload = $nameFile;
        // echo "tmpFile : ".$tmpFile."<br />";
        // echo "typeFile : ".$typeFile."<br />";
        // echo "nameFile : ".$nameFile."<br />";
        // if($typeFile != ""){
        ...
        // exit();
        $respon = json_decode($this->APICoreRKA->Apipost($this->url."/rab", $body));
        if($respon==null){
    ...
    function view(){
        ...
        $this->load->view('frame/a_header');
        $this->load->view('frame/b_nav',$data);
        $this->load->view('page/rab/rab_view',$data);
        $this->load->view('frame/d_footer');    
    }
